...
// Scan logs to find which agent is currently executing
const getActiveAgentStep = (logs) => {
  if (logs.length === 0) return 0;
  for (let i = logs.length - 1; i >= 0; i--) {
    const line = logs[i];
    if (line.includes('FactCheckerAgent') || line.includes('fact_check_task') || line.includes('Fact-check the SEO-optimized article')) {
      return 5; // Fact Checking
    }
    if (line.includes('SeoAgent') || line.includes('seo_task') || line.includes('Optimize the edited article')) {
      return 4; // SEO Optimizing
    }
    if (line.includes('EditorAgent') || line.includes('editing_task') || line.includes('Edit and refine the draft')) {
      return 3; // Editing
    }
    if (line.includes('WriterAgent') || line.includes('writing_task') || line.includes('Write a complete article')) {
      return 2; // Drafting
    }
    if (line.includes('ResearcherAgent') || line.includes('research_task') || line.includes('Research the following topic')) {
      return 1; // Researching
    }
    if (line.includes('QualityGate') || line.includes('quality gate') || line.includes('Evaluating quality gate')) {
      return 6; // Quality Gate Check
    }
  }
  return 1; // Default to Researching if job is running
};
...
const Sidebar = memo(function Sidebar({ activeTab, setActiveTab, apiOnline }) {
  return (
...
const WriteBriefForm = memo(function WriteBriefForm({ onSubmit, isGenerating, formMessage, jobs }) {
  // Localized states for form input to avoid global App re-renders when typing
        ...
        <form onSubmit={handleSubmit} style={{display: 'flex', flexDirection: 'column', gap: '1.25rem'}}>
          <div className="form-group">
            ...
            <input 
              type="text" 
              id="topic" 
              placeholder="Enter article topic (min 5 chars)..." 
              value={topic}
...
const LogsTerminal = memo(function LogsTerminal({ logs, autoscroll, setAutoscroll, setLogs }) {
  const consoleBoxRef = useRef(null);

  useEffect(() => {
    if (autoscroll && consoleBoxRef.current) {
...
const DocsViewer = memo(function DocsViewer({ docsContent, fetchDocs, handleRegenerateDocs }) {
  return (
    <div className="glass-panel">
      <div className="panel-header">
...
const JobInspectModal = memo(function JobInspectModal({ selectedJob, setSelectedJob }) {
...
const DashboardTab = memo(function DashboardTab({ 
  totalJobs, failedJobs, passRate, passingJobs, avgSeoScore, totalWords, 
  jobs, fetchJobs, setSelectedJob, logs, setActiveTab 
}) {
  const runningJob = jobs.find(j => j.status === 'running');
  const activeStep = runningJob ? getActiveAgentStep(logs) : 0;

  const pipelineStages = [
    { id: 1, label: 'Researching', desc: 'Researcher Agent gathering facts' },
    { id: 2, label: 'Drafting', desc: 'Writer Agent writing article' },
    { id: 3, label: 'Editing', desc: 'Editor Agent refining tone' },
    { id: 4, label: 'SEO Optimizing', desc: 'SEO Agent adding keywords' },
    { id: 5, label: 'Fact Checking', desc: 'Fact Checker verifying claims' },
    { id: 6, label: 'Quality Gate', desc: 'Auto checks & scoring' }
  ];

  return (
    <>
      {/* ── Active Production banner (pipeline & logs mini-console) ── */}
      {runningJob && (
        <div className="active-crew-card glass-panel status-running">
          <div className="active-crew-header">
            <div>
              <span className="badge running">
                <span className="pulsing-dot"></span>Active Production Pipeline
              </span>
              <h3 className="active-crew-title">Writing: "{runningJob.brief?.topic || runningJob.topic}"</h3>
            </div>
            <button className="btn btn-primary btn-sm" onClick={() => setActiveTab('logs')}>
              💻 View Live Terminal
            </button>
          </div>
            ...
            {pipelineStages.map((stage, idx) => {
              ...
              return (
                <React.Fragment key={stage.id}>
                  <div className={`pipeline-step ${isCompleted ? 'completed' : ''} ${isActive ? 'active' : ''} ${isPending ? 'pending' : ''}`}>
          ...
          <div className="micro-console">
            <div className="micro-console-header">Recent Agent Logs</div>
            <div className="micro-console-body">
              {logs.slice(-3).map((line, idx) => (
                <div key={idx} className="micro-console-line">{line}</div>
              ))}
              {logs.length === 0 && <div className="micro-console-line text-muted">Spawning agent threads...</div>}
            </div>
          </div>
        </div>
      )}
...
function App() {
  ...
  // Async states controlled in parent but used to regulate form and messages
  ...
  const fetchJobs = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // If there's an active running job, keep isGenerating true so UI knows production is active
  ...
  const handleSubmitBrief = async (briefData) => {
    if (!briefData.topic || briefData.topic.trim().length < 5) {
      setFormMessage({ type: 'error', text: 'Topic must be at least 5 characters long.' });
      return;
    }

    setIsGenerating(true);
    setFormMessage({ type: 'info', text: 'Content brief submitted. Initiating research crew in background...' });
    setActiveTab('logs'); // Auto-switch to logs tab to see agent activity

    try {
      const payload = {
        topic: briefData.topic,
        tone: briefData.tone,
        word_count: Number(briefData.wordCount),
        audience: briefData.audience,
        seo_keywords: briefData.seoKeywords ? briefData.seoKeywords.split(',').map(k => k.trim()) : [],
        reference_docs: briefData.referenceDocs ? [briefData.referenceDocs] : []
      };
      ...
      if (res.ok) {
        ...
        // Clear localized form fields
        if (briefData.clearForm) briefData.clearForm();
        setFormMessage({ type: 'success', text: `Job started successfully: ID ${result.job_id}` });
        
        // Immediately fetch jobs list
        fetchJobs();
      } else {
      ...
      <main className="workspace">
        ...
        {activeTab === 'docs' && (
          <DocsViewer 
            ...
            fetchDocs={fetchDocs}
            handleRegenerateDocs={handleRegenerateDocs}
          />
        )}
      </main>
